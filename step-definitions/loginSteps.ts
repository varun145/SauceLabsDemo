...
Given('User launches SauceDemo application', async function () {
...
Then('User should see Swag Labs logo', async function () {
  const logo = loginGlue.getSwagLabsLogo();
    await expect(logo).toContainText('Swag Labs');
  }
);

Then('User should see Products header', async function () {
