...
When('User clicks checkout button', async function () {
  checkoutGlue = new CheckoutGlue(this.page);
  await checkoutGlue.clickCheckout();
});

When('User enters firstname {string}', async function (firstname) {
  await checkoutGlue.enterFirstname(firstname);
});

When('User enters lastname {string}', async function (lastname) {
  await checkoutGlue.enterLastname(lastname);
});

When('User enters postalcode {string}', async function (postalcode) {
...
Then('User should see Overview page', async function () {
  const header = checkoutGlue.getOverviewHeader();
  await expect(header).toContainText('Checkout: Overview');
  }
);

Then('Product should be displayed in checkout overview', async function () {
...
Then('User should see order success message', async function () {
